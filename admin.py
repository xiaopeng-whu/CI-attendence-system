...
if __name__=="__main__":
       ...
       v_func = tk.IntVar()
       v_func.set(2)
       radiobutton_link = tk.Radiobutton(frame_func, text = '签到设置', variable = v_func, value = 1, indicatoron = False, padx = 50, command = lambda:showframe(check_setting))
       ...
       check_setting = tk.LabelFrame(root, width = 630, height = 400, borderwidth = 5, relief = tk.SUNKEN)
       check_setting.grid(row = 1, column = 1, rowspan = 2, columnspan = 2)
       frame.append(check_setting)
       tk.Label(check_setting, text='时间范围设置').place(x=150,y=60)
       tk.Label(check_setting, text='早：').place(x=150,y=90)
       var_morning = tk.StringVar()
       entry_morning = tk.Entry(check_setting, textvariable=var_morning)
       entry_morning.place(x=240,y=90)
       tk.Label(check_setting, text='中：').place(x=150,y=120)
       var_noon = tk.StringVar()
       entry_noon = tk.Entry(check_setting, textvariable=var_noon)
       entry_noon.place(x=240,y=120)
       tk.Label(check_setting, text='晚：').place(x=150,y=150)
       var_evening = tk.StringVar()
       entry_evening = tk.Entry(check_setting, textvariable=var_evening)
       entry_evening.place(x=240,y=150)
       tk.Label(check_setting, text='地点范围设置').place(x=150,y=180)
       tk.Label(check_setting, text='中心地点：').place(x=150,y=210)
       var_place = tk.StringVar()
       entry_place = tk.Entry(check_setting, textvariable=var_place)
       entry_place.place(x=240,y=210)
       tk.Label(check_setting, text='半径：').place(x=150,y=240)
       var_radius = tk.StringVar()
       entry_radius = tk.Entry(check_setting, textvariable=var_radius)
       entry_radius.place(x=240,y=240)
       #提交按钮
       check_submit = tk.Button(check_setting, text='submit', command=submit_info)
       check_submit.place(x=270,y=300)

       frame_check_signin = tk.LabelFrame(root, width = 420, height = 370, borderwidth = 5, relief = tk.SUNKEN)
       frame_check_signin.grid(row = 1, column = 1, rowspan = 2, columnspan = 2)
       ...
       frame_user_info.grid(row = 1, column = 1, rowspan = 2, columnspan = 2)
       ...
       frame_message_list.grid(row = 1, column = 1, rowspan = 2, columnspan = 2)
